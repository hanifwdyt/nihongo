...
// Level additions (not cumulative)
const hiraganaLevels: KanaEntry[][] = [
  // Level 1: Vowels + K + S rows
  [
...
const katakanaLevels: KanaEntry[][] = [
  [
...
function buildCumulative(levels: KanaEntry[][]): KanaEntry[][] {
  const result: KanaEntry[][] = [];
  let pool: KanaEntry[] = [];
  for (const additions of levels) {
    pool = [...pool, ...additions];
    result.push([...pool]);
  }
  return result;
...
export const hiraganaAdditions = hiraganaLevels;
export const katakanaAdditions = katakanaLevels;
export const hiraganaPools = buildCumulative(hiraganaLevels);
export const katakanaPools = buildCumulative(katakanaLevels);

export const TIMER_SECONDS = [300, 300, 360, 420, 480];
...
export function getPool(type: KanaType, level: number): KanaEntry[] {
  const pools = type === 'hiragana' ? hiraganaPools : katakanaPools;
  return pools[level - 1] ?? pools[0];
}

export function getAdditions(type: KanaType, level: number): KanaEntry[] {
  const additions = type === 'hiragana' ? hiraganaAdditions : katakanaAdditions;
  return additions[level - 1] ?? additions[0];
}
